...
API에서 입출력을 사용하기 위해서는 먼저 `framework.h` 헤더파일에 `#include <math.h>`를 작성해 수학 관련 헤더파일인 `math.h`를 넣어주도록 하자. 넣지 않으면 파일이 실행되지 않는 것은 아니지만, 수학적인 계산 에러 대부분은 해당 헤더 파일을 포함함으로써 해결된다.
...
`XBGR`은 앞 1`byte`를 비우고, 뒤에 `B`, `G`, `R`를 순서대로 입력하는 방식이다.

`ABGR`은 앞에 알파값을 채우고, 뒤에 `B`, `G`, `R`를 순서대로 입력하는 방식이다. `ABGR`의 `A`를 `Alpha`의 약자로 보면 될 것이다.
...
앞에서 말했다시피, 특정 도형의 전용 함수를 이용해 그림을 그릴 경우 도형의 안쪽은 비어있는 것이 아니라 흰색으로 채워져 있다. 따라서, 두번째로 그린 도형이 첫번째로 그린 도형보다 큰 경우, 처음 그린 도형은 완전히 가려지게 된다.

> *그렇다면 투명한 그림은 못 그리는가?*

아니다. 얼마든지 그릴 수 있다. 약간의 수학계산식 계산이 필요할 뿐이다. 단순히 곧게 뻗은 선분들이 이어져 만들어진 다각형은 단순한 `y = ax + b`수준의 식만 작성하고 응용할 줄 안다면 얼마든지 만들 수 있다.

문제는, 곡선이다. 선을 그릴 때에는 중등 수학만으로도 충분하지만, 곡선을 그려야 한다면 고등 수학이 들어가게 된다. 가장 좋은 예시로, 지금부터 할 "원 그리기"가 있다.

API에서 원을 그리는 가장 간단한 방법은, 지금까지 했던 방법대로 도형 전용 함수를 사용하는 것이다. 하지만 이 경우에는 도형 전용 함수는 
